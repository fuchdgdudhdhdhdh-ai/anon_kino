...
def kb_cart_prompt() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="➕ Добавить ещё один билет", callback_data="cart_add")
    builder.button(text="🍿 Перейти к закускам", callback_data="cart_finish")
    builder.button(text="❌ Отменить заказ", callback_data="menu:main")
    builder.adjust(1)
    return builder.as_markup()
...
async def cb_seat(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем корзину и очищаем временные поля текущего выбора,
    # чтобы при добавлении следующего билета не осталось "хвостов" от предыдущего.
    await state.update_data(
        tickets=tickets,
        movie_id=None,
        movie_title=None,
    ...
    await state.set_state(Booking.cart_review)

    tickets_lines = "\n".join(
        f"{i + 1}. 🎬 {t['movie_title']} — {t['session_time']}, зал «{t['hall']}», "
        f"ряд {t['row']}, место {t['seat']} — {t['price']} ₽"
        for i, t in enumerate(tickets)
    )
    tickets_total = sum(t["price"] for t in tickets)

    text = (
        f"🎫 <b>Билет №{len(tickets)} добавлен в корзину!</b>\n\n"
        f"🧺 <b>Ваша корзина:</b>\n{tickets_lines}\n\n"
        f"💰 Сумма за билеты: <b>{tickets_total} ₽</b>\n\n"
        "❓ <b>Хотите добавить ещё один билет?</b>"
    )
    await safe_edit(callback.message, text, kb_cart_prompt())
    await callback.answer()


@router.callback_query(F.data == "cart_add")
async def cb_cart_add(callback: CallbackQuery, state: FSMContext):
    """Клиент хочет ещё один билет — начинаем выбор заново с афиши, корзина сохраняется."""
    await state.set_state(Booking.choosing_movie)
    text = "🎞️ <b>Афиша на сегодня</b>\n\nВыберите фильм для следующего билета 👇"
    await safe_edit(callback.message, text, kb_poster())
    await callback.answer()


@router.callback_query(F.data == "cart_finish")
async def cb_cart_finish(callback: CallbackQuery, state: FSMContext):
    """Клиент закончил набирать билеты — переходим к выбору закусок."""
    await state.update_data(snacks=[])
    ...
    text = (
        f"🧺 Билетов в корзине: <b>{tickets_count}</b>\n\n"
...
async def cb_snacks_done(callback: CallbackQuery, state: FSMContext):
    ...
    total = tickets_total + snacks_total
    await state.update_data(total=total)

    text = (
        "🧾 <b>Подтверждение заказа</b>\n\n"
        f"🎟️ Билеты ({len(tickets)} шт.):\n{tickets_lines}\n\n"
        f"🍽️ Закуски:\n{snacks_lines}\n\n"
