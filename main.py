...
# ============================================================
#   ПРОВЕРКА ВРЕМЕНИ СЕАНСОВ (ПО ЧАСОВОМУ ПОЯСУ ЙОШКАР-ОЛЫ / МСК)
# ============================================================
...
def is_session_available(session_time_str: str) -> bool:
    """Проверяет, что сеанс ещё не начался (по текущему времени в Йошкар-Оле/МСК)."""
    now_time = datetime.now(CINEMA_TZ).time()
    return _parse_session_time(session_time_str) >= now_time


def get_available_sessions(movie: dict) -> list:
    """Возвращает список (индекс, сеанс) только для ещё не начавшихся сеансов фильма.
    Индекс сохраняется исходным (из полного списка сеансов), чтобы callback_data
    вида 'session:{movie_id}:{idx}' продолжал указывать на правильный сеанс."""
    return [
        (idx, session)
        for idx, session in enumerate(movie["sessions"])
        if is_session_available(session["time"])
    ]
...
def kb_poster() -> InlineKeyboardMarkup:
    ...
    for movie in content.MOVIES:
        has_sessions = bool(get_available_sessions(movie))
...
async def cb_movie_card(callback: CallbackQuery, state: FSMContext):
    ...
    available_sessions = get_available_sessions(movie)
    ...
    if available_sessions:
        ...
        text = header + (
            "😔 <b>На сегодня сеансов больше нет</b>\n"
            "Загляните завтра — расписание обновится!"
        )
...
async def cb_session(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_session_available(session["time"]):
        await callback.answer(
        ...
        text = header + (
            "🕒 Выберите сеанс 👇"
            if available_sessions
            else "😔 <b>На сегодня сеансов больше нет</b>\nЗагляните завтра — расписание обновится!"
        )
        await safe_edit(callback.message, text, kb_movie_card(movie, available_sessions))
        return
